...
class UserServiceTest {
    ...
    class UpdaterUserTest {
        ...
        @Test
        @SneakyThrows
        void when_update_user_without_changing_username_should_update_user_and_do_not_send_event() {

            UserUpdateDTO userUpdateDTOExpected = (UserUpdateDTO) userTestBuilder.withEmail("validUser123@email.com")
                    .withPassword("ValidPassword123!").withMatchingPassword("ValidPassword123!").build(ObjectType.UPDATE_DTO);

            UserProfileDocument userProfileExpectedBeforeUpdate = (UserProfileDocument) userProfileTestBuilder
                    .withAbout("").withInterests("").withLanguages("").withLocation("").withImage(null)
                    .build(ObjectType.DOCUMENT);
            UserDocument userExpectedBeforeUpdate = (UserDocument) userTestBuilder.withEmail("prevoius@email.com")
                    .withPassword("oldPass123!").withProfile(userProfileExpectedBeforeUpdate).build(ObjectType.DOCUMENT);
            UserProfileDocument userProfileExpectedAfterObjectMapping = (UserProfileDocument) userProfileTestBuilder
                    .withAbout("").withInterests("").withLanguages("").withLocation("").withImage(null)
                    .build(ObjectType.DOCUMENT);
            UserDocument userExpectedAfterObjectMapping = (UserDocument) userTestBuilder
